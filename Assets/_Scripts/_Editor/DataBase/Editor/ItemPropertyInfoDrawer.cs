...
public class ItemPropertyInfoDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (m_Properties != null && m_Properties.Length > 0)
        {
            ...
            nameProp.stringValue = EditorGUICustom.StringAtIndex(
                EditorGUI.Popup(popupRect, EditorGUICustom.IndexOfString(nameProp.stringValue, m_PropertyNames),
                    m_PropertyNames), m_PropertyNames);
            ...
            Rect descriptionRect =
                new Rect(position.xMax - position.width * 0.2f + EditorGUIUtility.standardVerticalSpacing, position.y,
                    position.width * 0.2f - EditorGUIUtility.standardVerticalSpacing, position.height);
            EditorGUI.LabelField(descriptionRect, "Type: " + propType.ToString().DoUnityLikeNameFormat(),
            ...
            else if (propType == ItemPropertyType.Float || propType == ItemPropertyType.Integer)
            {
                ...
                int selectedMode = GUI.Toolbar(selectModeRect, isRandomProp.boolValue == true ? 1 : 0,
                    new string[] { "Fixed", "Random" });
                isRandomProp.boolValue = selectedMode == 1 ? true : false;

                Rect valueRect = new Rect(selectModeRect.xMax + EditorGUIUtility.singleLineHeight, position.y,
                ...
                if (selectedMode == 0)
                {
                    if (propType == ItemPropertyType.Float)
                        ...
                        valueProp.floatValue =
                            Mathf.Clamp(EditorGUI.IntField(valueRect, Mathf.RoundToInt(valueProp.floatValue)), -9999999,
                                9999999);
                }
                else
                {
                    float[] randomRange = new float[]
                        { randomValueRangeProp.vector2Value.x, randomValueRangeProp.vector2Value.y };

                    if (propType == ItemPropertyType.Float)
                        randomValueRangeProp.vector2Value = EditorGUI.Vector2Field(valueRect, GUIContent.none,
                            randomValueRangeProp.vector2Value);
                    else
                        randomValueRangeProp.vector2Value = EditorGUI.Vector2IntField(valueRect, GUIContent.none,
                            new Vector2Int(Mathf.Clamp(Mathf.RoundToInt(randomRange[0]), -9999999, 9999999),
                                Mathf.Clamp(Mathf.RoundToInt(randomRange[1]), -9999999, 9999999)));
                }
            }
