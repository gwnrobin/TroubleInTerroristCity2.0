    ...
    public class LivingEntityEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            foreach (var field in fields)
            {
                ...
                if (fieldType.IsGenericType && fieldType.GetGenericTypeDefinition() == typeof(Value<>))
                {
                    ...
                    var currentValueField =
                        fieldType.GetField("m_CurrentValue", BindingFlags.NonPublic | BindingFlags.Instance);
            ...
            foreach (var field in fields)
            {
                ...
                if ((fieldType.IsGenericType && fieldType.GetGenericTypeDefinition() == typeof(Activity<>)) ||
                    fieldType == typeof(Activity))
                {
                    ...
                    EditorGUILayout.LabelField(field.Name.DoUnityLikeNameFormat() +
                                               (((bool)activeValue) ? " (Active)" : " (Inactive)"));
                }
            }
        }
    }
